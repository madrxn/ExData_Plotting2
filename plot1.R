#######################################################################
#
# Exploratory Data Analysis Project 2 - plot1.R
# Written By: madrxn (https://github.com/madrxn)
#
# GOAL: Use the base plotting system to explore if the total emissions 
#       for PM2.5 decreased in the United States from 1999 to 2008.
#
# plot1.R script does the following:
#   1) reads data PM2.5 emissions data (summarySCC_PM25.rds)
#   2) subsets Emissions and year column data
#   3) calculates total emissions for each year (1999,2002,2005,2008)
#   4) opens png device, creates "plot1.png" in working directory
#   5) generates plot of total PM2.5 Emissions by year and sends output  
#      to plot1.png file
#   6) closes png device
#
########################################################################

# loads dplyr
library(dplyr)

#1) reads in PM2.5 emissions data for 1999, 2002, 2005, and 2008
NEI <- readRDS("./summarySCC_PM25.rds")

#2) subsets based on Emissions and year columns
NEI_subset <- select(NEI, c(year, Emissions))

#3) groups subseted data by year and calculates the total emissions for each year
total <- NEI_subset %>%
         group_by(year) %>%
         summarize(Emissions = sum(Emissions))

#4) opens png device
png(filename = "./plot1.png", width = 640, height = 480, units = "px")

#5) plots total Emissiosn by year
with(total, plot(year, Emissions, 
                 type = "b", 
                 xlab = "Year", 
                 ylab = "Total PM2.5 Emissions (ton)", 
                 main = "Total PM2.5 Emissions in the United States\n From 1999, 2002, 2005, 2008",
                 sub = "PM2.5 Emissions in the US decreased from 1999 to 2008",
                 font.sub = 3))

#6) turns off png device
dev.off()